...
def main():
    ...
    for filename in os.listdir(CONTENT_DIR):
        if filename.endswith(".md"):
            ...
            all_posts.append({"title": post_title, "link": output_filename})

    # --- NEW: Build the index.html page ---
    print("Building index page...")
    index_links_html = "<ul>"
    for post in sorted(
        all_posts, key=lambda p: p["title"]
    ):  # Sort posts alphabetically
        index_links_html += (
            f'<li><a href="{post["link"]}" class="post-link">{post["title"]}</a></li>'
        )
    index_links_html += "</ul>"

    index_final_html = html_template.format(
        title="My Awesome Blog", content=index_links_html
    )
