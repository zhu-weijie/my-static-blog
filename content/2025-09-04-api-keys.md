...
tags:
  - api-keys
  - security
description: "A sequence diagram of how LLM services can use API keys to authenticate and authorize requests."
---

### Basic API key Authentication Pattern
...
sequenceDiagram
  ...
  Client->>API_Gateway: "GET /resource (Authorization: Bearer [API_KEY])"
  activate API_Gateway

  Note over API_Gateway: Extracts API key.
  API_Gateway->>Auth_Service: "Validate API Key"
  activate Auth_Service

  alt Key is Valid
    Auth_Service-->>API_Gateway: "{ valid: true, user_info }"
  else Key is Invalid
    Auth_Service-->>API_Gateway: "{ valid: false }"
  end
  deactivate Auth_Service

  Note over API_Gateway: Checks auth response.

  alt Authentication Succeeded
    API_Gateway->>Backend_Service: "Forward Request"
    activate Backend_Service
    Backend_Service-->>API_Gateway: "Service Response"
...
### Comprehensive API key Authentication Pattern
